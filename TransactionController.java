...
public class TransactionController {
    ...
    private void showRestockInventory() {
        try {
            VBox restockView = createRestockInventoryView();
            contentPane.getChildren().setAll(restockView);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    //------------------------------------------------------------------------------------------------------------------
    //CREATE VIEW FOR EACH TRANSACTION
    //------------------------------------------------------------------------------------------------------------------
    ...
    private VBox createRestockInventoryView() {
        ...
        Label subtitle = new Label("Track and log all restocking activities in the clinic inventory.");
        subtitle.setStyle("-fx-font-size: 13; -fx-text-fill: #7f8c8d; -fx-padding: 5 0 10 0;");
        ...
        TableColumn<RestockInventory, Integer> colRestockId = new TableColumn<>("Restock ID");
        colRestockId.setCellValueFactory(data -> new javafx.beans.property.SimpleObjectProperty<>(data.getValue().getRestockId()));
        colRestockId.setPrefWidth(90);

        TableColumn<RestockInventory, String> colItem = new TableColumn<>("Item");
        colItem.setCellValueFactory(data -> {
            try {
                ClinicInventory item = inventoryDAO.getAllItems().stream()
                        .filter(i -> i.getItemId() == data.getValue().getItemId())
                        .findFirst()
                        .orElse(null);
                return new javafx.beans.property.SimpleStringProperty(item != null ? item.getItemName() : "Unknown");
            } catch (Exception e) {
        ...
        TableColumn<RestockInventory, String> colPersonnel = new TableColumn<>("Restocked By");
        colPersonnel.setCellValueFactory(data -> {
            try {
                Integer personnelId = data.getValue().getRestockedBy();
                if (personnelId == null) return new javafx.beans.property.SimpleStringProperty("N/A");
                HealthPersonnel personnel = personnelDAO.getPersonnelById(personnelId);
                return new javafx.beans.property.SimpleStringProperty(
                        personnel != null ? personnel.getFirstName() + " " + personnel.getLastName() : "Unknown");
            } catch (Exception e) {
                return new javafx.beans.property.SimpleStringProperty("Error");
            }
        });
        ...
        TableColumn<RestockInventory, Integer> colQuantity = new TableColumn<>("Quantity Added");
        colQuantity.setCellValueFactory(data -> new javafx.beans.property.SimpleObjectProperty<>(data.getValue().getQuantityAdded()));
        colQuantity.setPrefWidth(120);
        ...
        TableColumn<RestockInventory, String> colDate = new TableColumn<>("Restock Date");
        colDate.setCellValueFactory(data -> new javafx.beans.property.SimpleStringProperty(
                data.getValue().getRestockDate() != null ? data.getValue().getRestockDate().toString() : "N/A"));
        colDate.setPrefWidth(150);

        table.getColumns().addAll(colRestockId, colItem, colPersonnel, colQuantity, colRemarks, colDate);
        ...
        Button btnRefresh = new Button("🔄 Refresh");
        btnRefresh.setStyle("-fx-padding: 10 20; -fx-pref-width: 150;");
    ...
    //Restocking Inventory Methods--------------------------------------------------------------------------------------
    private void loadRestockInventory(TableView<RestockInventory> table) {
        RestockInventoryDAO dao = new RestockInventoryDAO();
        List<RestockInventory> list = dao.getAll();
        table.setItems(FXCollections.observableArrayList(list));
    }
